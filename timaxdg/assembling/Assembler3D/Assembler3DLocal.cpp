...
void assemble_cell_curl(
  ...
  const dealii::FEValuesExtractors::Vector H(0);
  const dealii::FEValuesExtractors::Vector E(3);
  ...
  for (unsigned int q_point = 0; q_point < fe_v.n_quadrature_points; ++q_point)
	for (unsigned int i = 0; i < dofs_per_cell_E; ++i) {
	  ...
	  for (unsigned int j = 0; j < dofs_per_cell_H; ++j) {
		...
		// calculate 3D curl
		dealii::Tensor<1, 3> curl_H_j;
		curl_H_j[0] = grad_H_j[2][1] - grad_H_j[1][2];
		curl_H_j[1] = grad_H_j[0][2] - grad_H_j[2][0];
		curl_H_j[2] = grad_H_j[1][0] - grad_H_j[0][1];
...
void assemble_face_curl(
  ...
  for (unsigned int q_point = 0; q_point < fe_v.n_quadrature_points; ++q_point) {
	for (unsigned int i = 0; i < dofs_per_cell_E; ++i) {
	  ...
	  for (unsigned int j = 0; j < dofs_per_cell_H; ++j) {
		const auto H_j = fe_v[H].value(j, q_point);
		const auto n_times_H_j = dealii::cross_product_3d(normals[q_point], H_j);
	...
	for (unsigned int i = 0; i < dofs_per_cell_E; ++i) {
	  ...
	  for (unsigned int j = 0; j < dofs_per_cell_H; ++j) {
		...
		const auto n_times_H_neighbor_j =
			dealii::cross_product_3d(normals[q_point], H_neighbor_j);
	...
	for (unsigned int i = 0; i < dofs_per_cell_E; ++i) {
	  ...
	  for (unsigned int j = 0; j < dofs_per_cell_H; ++j) {
		...
		const auto H_neighbor_j = fe_v_neighbor[H].value(j, q_point);
		const auto n_times_H_neighbor_j = dealii::cross_product_3d(normals[q_point], H_neighbor_j);
...
void assemble_face_stab(
  ...
  const std::vector<double> &JxW = fe_v.get_JxW_values();
  const std::vector<dealii::Tensor<1, 3>> &normals = fe_v.get_normal_vectors();
  ...
  for (unsigned int q_point = 0; q_point < fe_v.n_quadrature_points; ++q_point) {
	for (unsigned int i = 0; i < dofs_cell; ++i) {
	  ...
	  const auto n_times_H_i = dealii::cross_product_3d(normals[q_point], H_i);
	  const auto n_times_E_i = dealii::cross_product_3d(normals[q_point], E_i);

	  for (unsigned int j = 0; j < dofs_cell; ++j) {
		const auto H_j = fe_v[H].value(j, q_point);
		const auto E_j = fe_v[E].value(j, q_point);
		const auto n_times_H_j = dealii::cross_product_3d(normals[q_point], H_j);
		const auto n_times_E_j = dealii::cross_product_3d(normals[q_point], E_j);
	...
	for (unsigned int i = 0; i < dofs_cell; ++i) {
	  ...
	  for (unsigned int l = 0; l < dofs_neighbor; ++l) {
		...
		const auto n_times_H_neighbor_l = dealii::cross_product_3d(normals[q_point], H_neighbor_l);
		const auto n_times_E_neighbor_l = dealii::cross_product_3d(normals[q_point], E_neighbor_l);
	...
	for (unsigned int k = 0; k < dofs_neighbor; ++k) {
	  ...
	  const auto n_times_H_neighbor_k = dealii::cross_product_3d(normals[q_point], H_neighbor_k);
	  const auto n_times_E_neighbor_k = dealii::cross_product_3d(normals[q_point], E_neighbor_k);
...
void mass_cell_worker_H(
  ...
  const dealii::FEValues<3> &fe_v = scratch_data.fe_values;
  const dealii::FEValuesExtractors::Vector H(0);
  ...
  for (unsigned int face_no = 0; face_no < dealii::GeometryInfo<3>::faces_per_cell; ++face_no) {
	...
	if (face->has_children()) {
	  ...
	  for (unsigned int subface_no = 0; subface_no < face->n_active_descendants(); ++subface_no) {
		...
		CurlCopyFaceData3D &copy_data_face = copy_data.face_data.back();
